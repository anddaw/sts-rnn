...
class _DenseNet(nn.Module):
    ...
    def __init__(self, in_features, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(in_features, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
...
class CNN2dDenseNet(BaseModel):

    def __init__(self, embedding_size: int, output_size: int, sentence_length: int = 30):
        super(CNN2dDenseNet, self).__init__(output_size)

        self.sentence_length = sentence_length
        self.dense_net = _DenseNet(embedding_size*2, block_config=(1, 1, 1, 1), num_classes=6)
